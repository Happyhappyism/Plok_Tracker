...
function apply_slot_data(slot_data)
	...
	if slot_data then
		if slot_data["fleasanity"] and slot_data["queen_items"] then
			if slot_data["fleasanity"] == 1 then
				local obj = Tracker:FindObjectForCode("Fleasanity")
				obj.Active = true
			end
			
			local treasure_needed = slot_data["queen_items"]
			local obj = Tracker:FindObjectForCode("Queen Items")
			
			obj.AcquiredCount = treasure_needed
		end
	end
end
